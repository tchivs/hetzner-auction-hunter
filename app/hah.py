...
class Server:
    ...
    def get_message(self, markdown=True, verbose=True):
        url = self.get_url()
        if not markdown:
            ...
            msg = f"**Hetzner** server fount: \n#ID: `{self.id}` \tin {self.datacenter}\n*{self.price}*€ monthly\n\n\n" + \
                  f"**{self.ram_size}GB RAM**\n{self.cpu_count}x `{self.cpu_description}`\n {self.disk_description}\n\n" + \
                  f"[Buy]({url})\n"
            if verbose:
                json_raw = json.dumps(self.server_raw, indent=4)
                msg += f"\n**Details**:\n```\n{json_raw}\n```\n"
    ...
    def __repr__(self):
        ...
        # Get Terminal Width
        terminalColumns = Analysis.__get_terminal_width__()
        ...
        for property in self.__dict__:
            ...
            if name not in excludeProperties:
                data.append(
                    [name, value, match_criteria, exclude_criteria, match_result, exclude_result, overall_result])
...
class Analysis(Server):
    ...
    ANALYSIS_FIELDS_DESCRIPTION = ["Matching Criteria", "Exclusion Criteria", "Match Result", "Exclude Result",
                                   "Overall Result"]
...
def send_notification(notifier, server, send_payload):
    if notifier == None:
        ...
        msg = server.get_message(
            False, send_payload).encode("utf-8")
        ...
        msg = server.get_message(html_supported, send_payload)
        title = server.get_header()
        if html_html:
            if title_subject:
                response = notifier.notify(message=msg, html=True, subject=title, **notifier_args)
            elif title_title:
                response = notifier.notify(message=msg, html=True, title=title, **notifier_args)
            else:
                response = notifier.notify(message=msg, html=True, **notifier_args)
        elif html_pamo:
            if title_subject:
                response = notifier.notify(message=msg, parse_mode=text_format, subject=title, **notifier_args)
            elif title_title:
                response = notifier.notify(message=msg, parse_mode=text_format, title=title, **notifier_args)
            else:  # telegram
                response = notifier.notify(message=msg, parse_mode=text_format, **notifier_args)
        else:
            if title_subject:
                response = notifier.notify(message=msg, subject=title, **notifier_args)
            elif title_title:
                response = notifier.notify(message=msg, title=title, **notifier_args)
            else:
                response = notifier.notify(message=msg, **notifier_args)
...
if __name__ == "__main__":
    ...
    for server_raw in servers:
        ...
        analysis.matchresult['disk_general_count'] = analysis.disk_general_count >= cli_args.disk_general_count[
            0] if cli_args.disk_general_count else True

        # Match General Total Disk Size
        analysis.matchresult['disk_general_total_size'] = analysis.disk_general_total_size >= \
                                                          cli_args.disk_general_total_size[
                                                              0] if cli_args.disk_general_total_size else True

        # Match General Each Disk Size
        analysis.matchresult['disk_general_each_size'] = analysis.disk_general_each_size >= \
                                                         cli_args.disk_general_each_size[
                                                             0] if cli_args.disk_general_each_size else True
        ...
        analysis.matchresult['disk_quick_count'] = analysis.disk_quick_count >= cli_args.disk_quick_count[
            0] if cli_args.disk_quick_count else True

        # Match Quick Total Disk Size
        analysis.matchresult['disk_quick_total_size'] = analysis.disk_quick_total_size >= \
                                                        cli_args.disk_quick_total_size[
                                                            0] if cli_args.disk_quick_total_size else True

        # Match Quick Each Disk Size
        analysis.matchresult['disk_quick_each_size'] = analysis.disk_quick_each_size >= cli_args.disk_quick_each_size[
            0] if cli_args.disk_quick_each_size else True

        # Match HDD Disk
        analysis.matchresult['disk_hdd'] = analysis.has_hdd_disk() if cli_args.disk_hdd else True

        # Match HDD Disk Count
        analysis.matchresult['disk_hdd_count'] = analysis.disk_hdd_count >= cli_args.disk_hdd_count[
            0] if cli_args.disk_hdd_count else True

        # Match HDD Total Disk Size
        analysis.matchresult['disk_hdd_total_size'] = analysis.disk_hdd_total_size >= cli_args.disk_hdd_total_size[
            0] if cli_args.disk_hdd_total_size else True

        # Match HDD Each Disk Size
        analysis.matchresult['disk_hdd_each_size'] = analysis.disk_hdd_each_size >= cli_args.disk_hdd_each_size[
            0] if cli_args.disk_hdd_each_size else True

        # Match SSD Disk
        analysis.matchresult['disk_ssd'] = analysis.has_ssd_disk() if cli_args.disk_ssd else True

        # Match SSD Disk Count
        analysis.matchresult['disk_ssd_count'] = analysis.disk_ssd_count >= cli_args.disk_ssd_count[
            0] if cli_args.disk_ssd_count else True

        # Match SSD Total Disk Size
        analysis.matchresult['disk_ssd_total_size'] = analysis.disk_ssd_total_size >= cli_args.disk_ssd_total_size[
            0] if cli_args.disk_ssd_total_size else True

        # Match SSD Each Disk Size
        analysis.matchresult['disk_ssd_each_size'] = analysis.disk_ssd_each_size >= cli_args.disk_ssd_each_size[
            0] if cli_args.disk_ssd_each_size else True

        # Match NVME Disk
        analysis.matchresult['disk_nvme'] = analysis.has_nvme_disk() if cli_args.disk_nvme else True

        # Match NVME Disk Count
        analysis.matchresult['disk_nvme_count'] = analysis.disk_nvme_count >= cli_args.disk_nvme_count[
            0] if cli_args.disk_nvme_count else True
        ...
        analysis.matchresult['disk_nvme_total_size'] = analysis.disk_nvme_total_size >= cli_args.disk_nvme_total_size[
            0] if cli_args.disk_nvme_total_size else True

        # Match NVME Each Disk Size
        analysis.matchresult['disk_nvme_each_size'] = analysis.disk_nvme_each_size >= cli_args.disk_nvme_each_size[
            0] if cli_args.disk_nvme_each_size else True
